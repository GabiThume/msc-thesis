...
\item[\sigla{CCV}{\textit{Color Coherence Vector}}:] captura informações sobre como as cores são organizadas em regiões conectadas, de acordo com um \textit{threshold} (Foi utilizado $T = 25$, sugerido pelos autores). Classifica os pixels da imagem em coerentes e incoerentes, computa os respectivos histogramas e os concatena \cite{ccv}. Dessa forma, o vetor de características produzido possui $2N$ dimensões.
...
Uma SVM encontra um hiperplano separador no espaço de características otimizando a máxima distância para o exemplo de treinamento mais próximo de cada classe. A versão atual do classificador, que permite sobreposição entre classes (também conhecida como versão \textit{soft-margin}), foi proposta por \cite{Vapnik e Cortes (1995}). O algoritmo utiliza exemplos de treinamento, resolvendo um problema de otimização formulado por meio de multiplicadores de Lagrange e um sistema de equações \textit{Karush Kuhn Tucker} (KKT).

Os exemplos de treinamento utilizados para computar o hiperplano são chamados vetores de suporte. Por isso, depois do treinamento todos os dados podem ser descartados, exceto os vetores de suporte que são utilizados para a etapa de classificação \cite{Marsland2015}.
...
% Essa redução permite a projeção dessas imagens no espaço de características.
