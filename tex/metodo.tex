...
também um certo borramento das bordas, como pode ser observado na Figura \ref{fig:gren:blur}.
...
O aguçamento visa enfatizar as transições de intensidade. Um processo bem conhecido
para atingir tal objetivo é utilizar unsharp mask. Ele borra a imagem, subtrai a
imagem borrada da original e adiciona essa diferença na imagem original (Ver Algoritmo
...
\begin{algorithm}[H]
  ...
  \ParaCada{pixel $(x,y)$}{
    ...
    $\text{G}(x,y) \gets I(x,y) + \text{diferença}$\;
  }
\end{algorithm}
\vspace{0.5cm}
...
causar o aparecimento de uma áurea em volta das bordas \cite{gonzalez}.
\item[Métodos relacionados]
