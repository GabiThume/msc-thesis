...
Neste capítulo os métodos de geração artificial são descritos.

Foram realizados experimentos que são posteriormente descritos no Capítulo \ref{cap:resultados} ao gerar essas imagens
utilizando borramento, mistura ponderada, unsharp masking, composição, combinação de
thresholds, combinação com saliência, SMOTE visual e adição de ruído.
Tendo quantas imagens são necessárias para rebalancear, foram geradas imagens de
cada método e também uma versão de todos eles.
...
Calcula a soma ponderada de duas imagens \ref{fig:gen:blend}.
